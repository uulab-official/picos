...
- The test is one-directional and named to say so. `reused` means the running process started after the preset was written and so is not the one saved; `consistent` is not the converse and records only that no contradiction was found; `unknown` covers a missing comparable time or a missing baseline.
- Two paths feed it and they are not equally strong, which a review caught before merge. Where the platform reports an absolute creation date, currently Windows, two wall-clock instants are compared and the result is sound. Elsewhere a kernel-measured `elapsed` is compared against `Date.now()`, so a forward clock step beyond the tolerance can report a false `reused`. The absolute path is therefore preferred when available, and the docs describe `reused` as strong evidence rather than proof.
- That same review found the absolute path was unreachable as first written: `parseWindowsProcessDetail()` populates `started` and never `elapsed`, so the one platform with an absolute time always reported `unknown`. The verdict now reads `started` first.
- The comparison tolerates two seconds, because `ps` reports elapsed time at one-second resolution, which biases the estimate toward a false `reused`, so the slack belongs on the recorded instant.
- `savedAtMs` is optional and never fabricated. An earlier revision made it required and defaulted it at construction, which quietly disabled the feature for presets written before the field existed: nothing persists a synthesized value, so it was re-derived on every invocation and always read as `consistent`. Leaving it absent lets the verdict report `unknown`, which is the truth. The clock now lives at the CLI save boundary, which also keeps the constructor deterministic.
...
- The stale-enumeration class was then checked against the new screen, since that is what adding a union member does to hand-written lists. Code was clean: both places that need a screen count read `screenOrder.length`, and the `1-9` shortcut list is correct because the new screen is twenty-second and has no shortcut. The README keyboard section was not: its `enter` entry listed what that key does and had gone stale, and its `j/k` entry was already loose before this change. Both now describe the shape rather than enumerate instances, which is the same fix as deriving a list from its union instead of hand-writing it, and `X` gained an entry because it had none.
- Following the same shape one step further found the worst instance of it. `refresh()` is scheduled with `setInterval`, which does not wait for the previous run, and its batch can exceed the default three-second interval on its own: the log snapshot alone allows three seconds, before `lsof`, `netstat`, the route table, and the inventory. So refreshes overlap routinely rather than exceptionally, and nothing stopped an older one from writing last. The display consequence is a stale summary or inventory winning. The worse one is that network timeline events are produced by diffing against `summaryRef`, so a late refresh diffed against a reference a newer refresh had already advanced and **recorded an interface change that never happened** as an audit event. `refresh()` now takes a sequence, awaits the inventory before writing anything so the batch lands atomically rather than pairing a fresh summary with a stale inventory, and on failure records the message in the event log while only the newest refresh may set the displayed error. The mechanism generalized from the process-inspection module into `src/tui/requestSequence.ts`, with one sequence per group of state that must stay consistent and separate sequences across groups.
- The rest of the seventh rule was then targeted by shape rather than by reading everything: the two defects already found in `App.tsx` were both in async long-running actions, so the remaining ones of that shape were read. Two of them write the same process detail and file snapshot state after awaiting a collector, and neither checked whether its own request was still the newest. With both in flight the slower one won, so the Processes workspace could show one process's detail beside another's open files, which is a wrong-conclusion hazard rather than a cosmetic one, and the window is real because `lsof` takes seconds. They now share one sequence in `src/tui/processInspection.ts`, checked before every publish including in the `catch`. Sharing is the point: separate counters would have left each writer considering itself current while overwriting the other.
- The seventh rule was then run against the one shape in `App.tsx` that could be found mechanically rather than by reading 18,200 lines: index clamping. Six sites did it inline in three different spellings. Two omitted the lower bound, so a negative index survived; one indexed an empty list at `-1`. All three happened to produce the right answer, each for a different reason, which is the kind of accident worth removing rather than depending on. They now call `clampIndex()` in `src/tui/navigation.ts`, which is tested, and `App.tsx` has no inline index arithmetic left. Twenty-seven similar sites remain in other `src/tui` modules and were deliberately left alone: those modules have tests, so their edge behaviour is already pinned, and converting them would be churn without reducing risk. The rule stays open for the rest of `App.tsx`, which cannot be audited by pattern.
- Two further rules were run and found nothing, which is worth recording with the reason so the clean result is not re-derived. Every `?? new Date()` and defaulted `now` under `src/` describes the thing being created, a `generatedAt` on an audit export plan or the start instant of a run, rather than inventing a past instant that should have come from storage; the `savedAtMs` defect was the latter and is the only one of its kind. And no absolute guarantee language survives in `docs/LOCAL_AUTOMATION.md`, the claims corrected earlier in this work having been the last of them. Six of the seven rules have now been run as audits; the seventh, moving decisions out of components, applies to the whole of `App.tsx` and stays open.
- Two more of the rules were then run as audits. The advertised-key rule found one defect: the Files provider boundary row offered `c connect from Remotes`, but that row renders inside Files where `c` is the copy operation, so a reader following the hint got a copy dialog. It now names the workspace instead of a key, and the rule in `AGENTS.md` was sharpened to say cross-workspace rows must name a workspace or command rather than a bare letter. The missing-status-set rule found nothing further: `requestReadOnlySftpConnectionCancellation()` compares only `connecting`, which is correct, because a second cancel should be a no-op and concurrent connects are handled by superseding the pending controller rather than refusing to start. Also checked and correct: the Status queue control row lists five sources while `StatusActivitySource` has six, because `timeline` is a result source rather than a queue source; it is unreachable by cycling and its empty branch is defensive. That union serves two roles, so do not "fix" the row by adding `timeline` to the queue list.
- The rules that came out of the review were then used as an audit, on the theory that a defect class proven to exist here is worth searching for rather than waiting for. Two of the seven found nothing: no boolean is used as an in-flight flag anywhere else, and the SFTP connect flow already compares controller identity rather than a flag. One found a live defect. Five long-running actions share the `commandStatus` indicator and three reset it unconditionally in a `finally`, so with any two overlapping, the first to return cleared it while the second was still working. It is now reference counted in `src/tui/commandStatus.ts`, with the end unconditional so a superseded action still balances the count and the count floored at zero so an unbalanced end cannot strand the indicator on running.
- The decisions that produced those defects were then moved out of the component. `canStartOperationRun()`, `selectOperationPreset()`, and `finishMonitorOperationRun()` live in `src/tui/operationRunPanel.ts` with tests, and `App.tsx` keeps only the wiring and the collector calls. This is the repo's existing shape rather than a new one: every other row formatter and state machine already sits outside `App.tsx`, which is why they were covered and these were not. The rule is now written down in `AGENTS.md` so the next workspace does not repeat it. Introducing Ink component tests was considered and rejected for this: none of the defects were in rendering.
- A review before merge found three defects the gate could not, because no test touches `App.tsx`. Runs are now identified by a token rather than tracked with a shared boolean: `cancelling` is an in-flight status that the old guard read as idle, so `X` then `enter` started a second run whose first statement un-cancelled the first, after which the superseded loop sampled out its full window and published onto the second run's progress. Cancellation is no longer advertised for a single-sample monitor run, which has no between-samples point at which to observe it. And the selection index is clamped, because a preset removed through the CLI while the TUI is open left the cursor marking nothing and `enter` reporting no selection.
