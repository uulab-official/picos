...
- Where two workspaces share a binding, derive both hint rows from one table rather than writing each row by hand. Connections and Ports bind nearly the same keys, and the hand-written rows drifted: `enter` runs process inspection on both, the Ports row said so and the Connections row did not, so the feature was undiscoverable from one of them. Scope the entries that genuinely belong to one screen instead of duplicating the whole row. This makes the two rows unable to disagree, but it does not prove a key is bound, because dispatch still lives in `useInput`; that check remains manual.
...
- The `LocalInspectorCommand` guard must be derived from an exhaustive `Record` over the union, never hand-listed as a chain of comparisons. The hand-written chain did not fail to compile when a command was added to the union, so the new command's argv parse failures silently fell back to a non-JSON error. Test the guard through `Object.hasOwn`, not `in`, because `in` reports inherited keys and would accept `toString` as a command.
- When a listing is bounded twice, publish each bound separately and name what it means. The handoff index is capped by the requested read limit and again by the JSON byte budget; collapsing those into one `truncated` flag would leave a consumer unable to tell "more files exist on disk" from "rows were dropped to fit".
