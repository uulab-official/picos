...
- Process presets are the one kind that stores an ephemeral identifier, which the published contract labels as such. A PID is only valid while that process lives, and the OS may reuse it, so a re-run can succeed while describing an unrelated process. Running a process preset therefore reports `data.identity`, and a saved preset records the instant it was written as `savedAtMs`. The check needs no absolute process start time and so works on every platform: if the running process is younger than the preset, it cannot be the process that was saved. Read the three values as follows.
  - `reused` is a **proof of difference**. The process now holding that PID started after the preset was written, so it is not the one you saved. Re-save the preset against the current PID rather than trusting the reading.
  - `consistent` is **not** proof of sameness. It only means no contradiction was found: the process is older than the preset, which is what you would see either way if the PID was reused by a long-lived process. It raises confidence without settling the question.
  - `unknown` means there was no elapsed time to compare, so no conclusion is available.
  - The comparison tolerates two seconds, because `ps` reports elapsed time at one-second resolution and a preset saved immediately after a process started would otherwise read as reused.
  - The plain-text form prints an `identity=reused` line only for the proof case, since that is the only one that should change what a reader does.
