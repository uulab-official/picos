...
- Process presets are the one kind that stores an ephemeral identifier, which the published contract labels as such. A PID is only valid while that process lives, and the OS may reuse it, so a re-run can succeed while describing an unrelated process. Detecting that is platform-dependent: `data.detail.started` carries an absolute creation date on Windows and can be compared across runs, while the macOS and Linux collectors populate only `data.detail.elapsed`, where reuse can be inferred but not proven from an elapsed time shorter than the age of the preset. Treat a saved process preset as a short-lived convenience rather than a durable handle; monitor and logs presets have no such issue because they store a policy rather than an identity.
