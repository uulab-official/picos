...
- [x] 상태: 완료 — Status evidence의 handoff·audit·cleanup·Tools·process·remote known_hosts·interface·Timeline·status-activity export/open/archive/retention/recovery 전이를 순수 owner로 분리하고, 독립 갱신 그룹 7개에 별도 request sequence를 적용해 stale 성공·실패가 현재 index/selection/error를 publish하지 않도록 검증
