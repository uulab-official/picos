...
- [x] 상태: 차단 기록 완료 — `git diff --check` 통과. `bun run verify`, `bun test`, `bun run typecheck`는 현재 환경에 Bun/로컬 런타임이 없어 실행 차단
...
- [ ] 완료 조건 — `bun run verify`, lint, typecheck, build, smoke 및 지원 harness가 통과

## 우선순위

1. release/version 상태 동기화
2. App.tsx 구조 및 테스트 공백
3. 안전한 local write 기능의 회귀 검증 및 문서화
4. plugin registry와 Docker/SSH 확장
5. 실제 OS mutation 및 SFTP remote write
