...
- [x] 상태: 완료 — Actions의 catalog lookup·metadata/risk/privilege/lock·preview/adapter/platform·exact confirmation·control execution publication을 `actionControlTransitions.ts` 순수 전이로 분리하고, enabled read는 기존 feature owner로 직접 전달하며 mutable action은 완전한 adapter-owned dry-run preview만 허용; 공유 request token이 stale control 성공·실패를 현재 상태와 분리하고, Connections/Ports hint는 현재 filtered/sorted selection에서 실제로 동작하는 PID control과 이름 있는 Processes/CLI handoff만 표시
