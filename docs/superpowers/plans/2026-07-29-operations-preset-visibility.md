...
Those three were UX decisions rather than engineering ones, so they were written up separately as options with precedent and recommended defaults in [2026-07-29-operations-workspace-options.md](2026-07-29-operations-workspace-options.md). All four were agreed and the workspace has since been built, so the blocker recorded here is resolved. That document also records why monitor cancellation is partial-result based while the SFTP session is error-based, since the two modules would otherwise look inconsistent.

The Config workspace decision in this slice still stands and was not revisited: presets remain **coverage** in Config rather than a managed shelf, because `picos operations` is still their only editor. The new Operations workspace runs presets and cannot create, edit, or remove one, so no `ConfigManagedShelfTarget` was added after all.
