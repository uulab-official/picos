...
- `?` or `/`: open the command palette, then type to filter commands, including `tools dns`, `tools tls`, `tools traceroute`, or `telnet` for direct Tools target prompts with live field-form target/default/placeholder/CLI previews, `remote known_hosts select` or `known hosts candidate` for the Remotes typed known_hosts candidate prompt, `trail` for recovered Timeline Evidence trail select/open/search actions, `trail source` for source filtering, `process evidence` / `process evidence open` / `process evidence search` for recovered process-control evidence actions, `known_hosts evidence` / `known_hosts evidence open` / `known_hosts evidence search` / `known_hosts evidence copy` / `known_hosts evidence export` / `known_hosts handoff select` for recovered Remotes selection-history evidence actions and handoff-result selection, `result jump` / `timeline result open` / `result select` / `result jump filter` / `jump class` for Status result Timeline jump controls with compact previews for filter/query/process PID targets, `jump class config` / `status jump config` for focusing the saved Config result-jump-class row, `safety policy config` / `editor save config` / `audit retention config` / `tools retention config` for jumping to OS settings rows with current-value previews, `route filters config` / `connection filters config` / `port filters config` / `tools shelf config` / `tool target presets config` / `log profiles config` / `remote profiles config` for Config-origin managed shelf landings with live saved-count previews, `empty route filters` / `recover connection filters` / `missing port filters` / `recover tools targets` / `missing log profiles` / `recover remote profiles` for Config recovery previews and dispatch into the owning shelf workspace, `result filter` / `palette result jumps` / `known_hosts handoffs` for Status Activity result-history filtering, and `tools evidence` / `tools search` / `tools compare` / `tools match` / `tools archive` / `tools retention` for Tools evidence search, recovered-match open/archive, scope filtering, archive, and retention prompts
