...
- Editor workspace: opened text files keep their original buffer content and render a locked `EDITOR SAVE PREVIEW` with provider, path, write risk, exact `save file` phrase, changed-line counts, and diff rows before any filesystem write path is allowed to execute
