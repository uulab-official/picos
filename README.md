...
- `?` or `/`: open the command palette, then type to filter commands, including `tools dns`, `tools tls`, `tools traceroute`, or `telnet` for direct Tools target prompts with live field-form target/default/placeholder/CLI previews, `remote known_hosts select` or `known hosts candidate` for the Remotes typed known_hosts candidate prompt, `trail` for recovered Timeline Evidence trail select/open/search actions, `trail source` for source filtering, `process evidence` / `process evidence open` / `process evidence search` for recovered process-control evidence actions, `known_hosts evidence` / `known_hosts evidence open` / `known_hosts evidence search` / `known_hosts evidence copy` / `known_hosts evidence export` for recovered Remotes selection-history evidence actions, `result jump` / `timeline result open` / `result select` / `result jump filter` / `jump class` for Status result Timeline jump controls with compact previews for filter/query/process PID targets, `jump class config` / `status jump config` for focusing the saved Config result-jump-class row, `safety policy config` / `editor save config` / `audit retention config` / `tools retention config` for jumping to OS settings rows with current-value previews, `route filters config` / `connection filters config` / `port filters config` / `tools shelf config` / `tool target presets config` / `log profiles config` / `remote profiles config` for Config-origin managed shelf landings with live saved-count previews, `empty route filters` / `recover connection filters` / `missing port filters` / `recover tools targets` / `missing log profiles` / `recover remote profiles` for Config recovery previews and dispatch into the owning shelf workspace, `result filter` / `palette result jumps` for Status Activity result-history filtering, and `tools evidence` / `tools search` / `tools compare` / `tools match` / `tools archive` / `tools retention` for Tools evidence search, recovered-match open/archive, scope filtering, archive, and retention prompts
...
- Status Activity result history supports `f result filter` to cycle between `all` and `palette-result-jumps`; filtered rows keep original row numbers and `u`/`i` navigation stays inside the visible filtered list, while palette-triggered filter changes append compact `palette status result filter ...` rows with visible/total counts
...
- Remote known_hosts evidence actions: exported Remotes known_hosts selection-history audit files now appear as a first-class `remote-known-hosts` family in Status Evidence after audit refresh or boot, can be focused with `Tab`/`1..9`, moved with `[`/`]`, opened through locked `:file-open` with `enter`/`R`, searched in Timeline with `G`, copied through locked `:clipboard`, exported as a selected audit handoff, and selected/opened/searched/copied/exported through command palette `known_hosts evidence` actions; palette and Status Evidence searches both preview the selected export target/query/path, append Status Activity result rows, and leave searchable Timeline audit events that can be replayed from result-history `I` while retaining the no-local-read/no-network/no-scan/no-trust/no-write posture.
