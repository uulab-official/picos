...
Press `T` in the Tools workspace to persist the active target preset, `L` to rename the selected saved target preset, and `X` to remove it. OS-aware presets such as gateway, DNS server, and default host remain generated from the current machine state. Saved target presets are trimmed, de-duplicated by action and target, capped at eight entries, and restored before OS-aware presets on the next TUI boot.
