...
- DNS workspace: shows configured resolvers and supports `S` to open a locked DNS server proposal prompt, where typed resolver IPs are deduplicated, validated, compared against current servers, and rendered with preflight/rollback rows; `C` clears the proposal, and execution remains disabled so no DNS setting is changed
