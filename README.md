...
- Tools workspace: `n` cycles OS-aware and saved target presets, `T` saves the active target preset to config, `L` renames the selected saved target preset, `M` edits the selected saved target value, `X` deletes the selected saved target preset, `R` runs the selected preset, `j/k` selects previous runs, `Tab` cycles raw/summary/command detail panes, `f` filters history, `P` saves the active filter to config, `]` cycles saved presets, `s` cycles sorting, `G` groups by tool/action or status, `r` reruns, `y` copies summaries, `c` copies raw output, and `e`/`E` exports selected/all runs
...
Press `T` in the Tools workspace to persist the active target preset, `L` to rename the selected saved target preset, `M` to edit its target value, and `X` to remove it. OS-aware presets such as gateway, DNS server, and default host remain generated from the current machine state. Saved target presets are trimmed, de-duplicated by action and target, capped at eight entries, and restored before OS-aware presets on the next TUI boot.
