...
- Empty Config recovery palette actions for Routes, Connections, Ports, and Logs now land in the owning workspace and open the matching `route-filter`, endpoint-filter, or `log-search` prompt immediately, so missing OS-console presets can be created without a second Enter.
...
- Config recovery actions skip the extra destination Enter for empty route filters, endpoint filters, port filters, and log profiles by opening their creation/search prompt as soon as the palette action dispatches.
- `d`: run doctor
