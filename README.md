...
- Tools evidence search changes append compact Status Activity result rows and searchable Timeline audit messages with active/archive target, query, and visible/total match counts; palette-triggered Tools evidence archive/retention prompts append compact Status Activity result rows, and confirmed archive/prune submissions leave outcome rows with blocked/archived/pruned status for later copy, export, or Timeline recovery
...
- Tools Hub with read-only DNS/RDAP/IP/TCP/TLS/ping/traceroute commands, target prompts, OS-aware and saved target presets, filterable/sortable/groupable/selectable result history, raw/summary/command/compare detail panes, session filter presets, rerun, locked summary/raw-output/TCP-section/TCP-row/compare copy, scoped markdown export including `scope=compare`, indexed Status Evidence recovery with scope and token search filters, search-change Status Activity/Timeline audit recovery, archive/retention management, and raw output handoff
