...
- `?` or `/`: open the command palette, then type to filter commands, including `trail` for recovered Timeline Evidence trail select/open/search actions, `trail source` for source filtering, `process evidence` / `process evidence open` / `process evidence search` for recovered process-control evidence actions, `result jump` / `timeline result open` / `result select` / `result jump filter` / `jump class` for Status result Timeline jump controls with compact previews for filter/query/process PID targets, `jump class config` / `status jump config` for focusing the saved Config result-jump-class row, `safety policy config` / `editor save config` / `audit retention config` / `tools retention config` for jumping to OS settings rows with current-value previews, `route filters config` / `connection filters config` / `port filters config` / `log profiles config` / `remote profiles config` for Config-origin managed shelf landings, `result filter` / `palette result jumps` for Status Activity result-history filtering, and `tools evidence` / `tools search` / `tools compare` / `tools match` / `tools archive` / `tools retention` for Tools evidence search, recovered-match open/archive, scope filtering, archive, and retention prompts
