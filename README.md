...
- Status Activity result history supports `f result filter` to cycle between `all` and `palette-result-jumps`; filtered rows keep original row numbers and `u`/`i` navigation stays inside the visible filtered list
