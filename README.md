...
- Connections and Ports workspaces: show parsed rows plus clipped raw OS command output; `j/k` selects endpoints, `s` cycles endpoint sorting, PID matches show process snapshots, and `c` opens a copy preview
- `d`: run doctor
