...
- Remotes workspace: `enter` opens remote focus, `j/k` selects configured SFTP profiles, selected profiles show `REMOTE HANDOFF` rows with computed remote root, locked writes, staged/not-staged state, and no-network-session posture, and `enter` stages a locked remote file context for Files without connecting yet
...
- `picos remote <id>`: inspect the remote provider and handoff boundary for a configured profile without opening a network session.
