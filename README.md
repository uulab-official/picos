...
- Status Activity result history supports `f result filter` to cycle between `all`, `palette-result-jumps`, and `evidence-handoffs`; filtered rows keep original row numbers and `u`/`i` navigation stays inside the visible filtered list, while `H` jumps between Remotes `known_hosts` evidence copy/export handoff rows from the copy-intent shelf and `I` replays the selected handoff Timeline search; palette-triggered filter changes append compact `palette status result filter ...` rows with visible/total counts
