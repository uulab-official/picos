...
- Tools evidence search changes append compact Status Activity result rows and searchable Timeline audit messages with active/archive target, query, and visible/total match counts; selected search result jumps appear in the Status copy-intent shelf as `tools search target=... query=...` rows, and reusable search audit jumps keep compact target/query tokens; palette-triggered Tools evidence archive/retention prompts append compact Status Activity result rows, and confirmed archive/prune submissions leave outcome rows with blocked/archived/pruned status for later copy, export, or Timeline recovery
