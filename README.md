...
- Config workspace: settings are grouped into retention, display, connectivity, and safety sections; `1..4` jumps to a section, the detail pane shows the selected section, config path, active safety posture, and persistence hint, `j/k` selects rows, `+/-` adjusts and persists numeric, language, boolean, and dry-run policy values, `enter` edits `defaultPingHost` or logs the current config path and active values, `P` cycles safe/user dry-run/admin dry-run policy presets, and `R` opens an exact `reset config` confirmation preview for restoring core controls to defaults
- `d`: run doctor
