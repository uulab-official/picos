...
- Interfaces workspace: `j/k` selects interfaces, `Tab` cycles list/detail/stats/platform/source panes, `D` opens a locked disable proposal, `U` opens a locked enable proposal, and `C` clears that proposal; command-palette `interface proposal disable` and `interface proposal enable` open the same selected-adapter proposal with preview rows before dispatch; every pane keeps a selected-interface strip visible with group, primary marker, IPv4/IPv6, MAC/netmask, MTU, RX/TX counters, gateway, DNS, public IP, platform source, and safe action hints, while the source pane shows raw OS inventory/stat/gateway/DNS command sources, retained bounded source-output snippets for inventory/stats/gateway evidence, locked source evidence copy/export/open handoffs with `c`/`e`/`o`, plus the locked adapter-owned `interface.disable` preview, and command-palette `interface.disable` previews include the selected adapter target before opening the locked Action Center view
