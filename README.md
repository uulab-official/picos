...
- `?` or `/`: open the command palette, then type to filter commands, including `trail` for recovered Timeline Evidence trail select/open/search actions, `trail source` for source filtering, `result jump` / `timeline result open` / `result select` for Status result Timeline jump controls, and `result filter` / `palette result jumps` for Status Activity result-history filtering
