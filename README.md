...
- Palette-triggered `trail source` cycles also append Status Activity result history rows and searchable Timeline audit events with the selected source filter and visible/total recovered trail count; select that result with `u`/`i`, then press `I` to jump into the matching Timeline audit search, record that jump as a reusable copy-intent row, surface the latest jump as an `audit jump intent=` hint with a repeat count beside Status results, keep an `audit jumps count=... latest=... I=fresh/replay` shelf summary visible in Status copy intents, and replay that latest jump with `I` when the selected Status result row cannot create a fresh jump
