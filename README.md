...
- Status Activity result history supports `f result filter` to cycle between `all`, `palette-result-jumps`, and `evidence-handoffs`; filtered rows keep original row numbers and `u`/`i` navigation stays inside the visible filtered list, while palette-triggered filter changes append compact `palette status result filter ...` rows with visible/total counts
