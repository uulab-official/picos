...
Ports process control follows the same locked-first model. Pressing `K` on a selected listening port shows the target port, PID, process, user, exact phrase such as `kill pid 12345`, and a dry-run lock row; the adapter-owned process termination command remains disabled by default.
