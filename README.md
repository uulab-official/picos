...
- `picos remote <id>`: inspect the remote provider, handoff boundary, adapter boundary, transport probe, read-only adapter contract, locked file request preview, host-key evidence, host-key scan request, known_hosts source/read preview/read result/parser/candidate previews, host-key trust decision preview, host-key compare detail with selected candidate metadata, host review, and connect preview for a configured profile without opening a network session.
...
The Files workspace supports keyboard-driven local navigation, numbered system location jumps, direct path input, entry filtering, locked file-operation previews, SFTP-aware breadcrumbs, and read-only file preview into the Editor workspace. The Remotes workspace surfaces configured SFTP-style profiles, shows locked handoff, recent activity, adapter, transport-probe, read-only adapter contract, file request preview, host-key evidence, host-key scan request, known_hosts source/read/read-result/parser/candidate previews, host-key trust decision, compare-detail with selected candidate metadata, scan-review/trust-review activity, host-review, and connect-preview boundaries before staging, records searchable Timeline and Status Activity evidence, lets you stage a locked remote file context, and never opens network sessions yet.
