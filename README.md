...
- Palette-triggered `trail source` cycles also append Status Activity result history rows and searchable Timeline audit events with the selected source filter and visible/total recovered trail count; select that result with `u`/`i`, then press `I` to jump into the matching Timeline audit search, record that jump as a reusable copy-intent row, and surface the latest jump as an `audit jump intent=` hint beside Status results
