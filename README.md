...
- Status Activity result history supports `f result filter` to cycle between `all`, `palette-result-jumps`, and `evidence-handoffs`; filtered rows keep original row numbers and `u`/`i` navigation stays inside the visible filtered list, while `H` jumps between Remotes `known_hosts` evidence copy/export handoff rows from the copy-intent shelf and `I` replays the selected handoff Timeline search; palette-triggered `known_hosts handoff open` replays now leave a dedicated copy-intent row with target id, action, source row, match count, `g Timeline`, and `v replay` hints
