...
Ports process control follows the same locked-first model. Pressing `K` on a selected listening port opens a `:port-control` prompt with the target port, PID, process, user, exact phrase such as `kill pid 12345`, and a dry-run lock row. Entering the exact phrase records a disabled confirmation audit event; rejected input records a rejected audit event. The adapter-owned process termination command remains disabled by default.
