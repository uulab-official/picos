...
- Files workspace: `enter` opens file focus, `j/k` selects entries, `enter` opens, `g` cycles system locations, `u` goes to the parent directory, and `h`/`esc` returns to workspace navigation
- `d`: run doctor
...
The Files workspace now supports keyboard-driven local navigation, system location jumps, and read-only file preview into the Editor workspace.
