...
- Editor workspace: opened text files keep their original buffer content, `j/k` moves the selected line, `a` appends a line, `r` replaces the selected line, `x` deletes the selected line in memory, buffer state shows dirty/line/truncation counts, `s` opens an exact `save file` confirmation, and the locked `EDITOR SAVE PREVIEW` shows provider, path, write risk, changed-line counts, and diff rows before any filesystem write path is allowed to execute
