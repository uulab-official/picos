...
describe("remote profiles", () => {
	...
	test("formats remote host key compare detail without reading or trusting", () => {
		...
		expect(
			...
			"candidateSource=none hostPattern=none keyType=unknown",
	...
	test("formats remote host key compare detail with selected read-result candidate", () => {
		const profile = {
			id: "prod",
			kind: "sftp" as const,
			host: "prod.example.com",
		...
		expect(detail).toMatchObject({
			...
			selectedHostPattern: "[prod.example.com]:2222",
			selectedKeyType: "ssh-rsa",
			match: "unknown",
			decision: "blocked",
			confirm: "review host trust prod",
			execution: {
		...
		expect(detail.knownHostsCandidateFingerprint).toEqual(
			expect.stringMatching(/^SHA256:[A-Za-z0-9+/]+$/),
		);
		expect(formatRemoteHostKeyCompareDetailRows(detail)).toEqual([
			...
			"candidateSource=local-known-hosts-read-result hostPattern=[prod.example.com]:2222 keyType=ssh-rsa",
			'match=unknown decision=blocked confirm="review host trust prod"',
			"execution=willImport=false willConnect=false willReadLocal=false willParse=false willScan=false willTrust=false willMutate=false",
