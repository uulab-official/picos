...
- Running a saved process preset now reports whether the PID was reused. A process preset records the instant it was saved as `savedAtMs`, and a run compares that against the running process's elapsed time to publish `data.identity` as `reused`, `consistent`, or `unknown`; the plain-text form prints an `identity=reused` line for the proof case only. Only `reused` is a proof, and it proves difference: a process younger than the preset cannot be the one that was saved. That makes reuse detectable on macOS and Linux, which expose only elapsed time, rather than on Windows alone. The comparison tolerates two seconds because `ps` reports elapsed time at one-second resolution. Presets saved before this field existed are stamped on first load, which loses nothing, since an unknown baseline can only ever read as consistent.
