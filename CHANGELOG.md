...
- SFTP transport readiness can now consume injected `REMOTE SFTP PACKAGE RESOLUTION REVIEW` metadata, so a found optional transport package satisfies the `transportInstalled` host-key scan readiness check while resolver execution, package reads, imports, sockets, sessions, trust writes, and mutation remain disabled.
...
- Remotes workspace and `picos remote <id>` now show `REMOTE HOST KEY SCAN POLICY` rows after the scan request, keeping the future host-key scan path disabled by default while exposing preview-only mode, SFTP dependency, scan review/transport/host-review/known_hosts-compare prerequisites, policy/transport/fingerprint blockers, and no-import/no-connect/no-scan/no-trust/no-known_hosts-write/no-mutation execution flags.
